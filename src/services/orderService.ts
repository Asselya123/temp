import { Order, Plan, Certificate, OrderStatus } from '../types';
...
export const orderService = {
  ...
  createOrder: async (orderData: Omit<Order, 'id' | 'status' | 'plan'> & { planId: string, certificateCode?: string }): Promise<Order> => {
    // Simulate API call delay
    await new Promise(resolve => setTimeout(resolve, 500));
    ...
    if (orderData.certificateCode) {
      certificate = certificates.find(c => c.code === orderData.certificateCode && !c.isUsed);
      if (certificate) {
  ...
  createCertificate: async (code: string, duration: number, expiryDate: Date): Promise<Certificate> => {
    // Simulate API call delay
    await new Promise(resolve => setTimeout(resolve, 500));
